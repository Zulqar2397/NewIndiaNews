...
public class NewsServiceImpl implements NewsService {
	...
	public NewsDto addNews(NewsDto newsDto) throws ServiceException {
		...
		news.setTime(new java.sql.Time(today.getTime()));
	...
	public List<NewsDto> convertEntityToDTO(List<News> news) {
		...
		news.forEach(item -> newsList.add(new NewsDto(item.getNewsId(), item.getTitle(), item.getDescription(),
				item.getRegion(), item.getSource(), item.getLikeCount(), item.getDate(), item.getTime(),
				item.getHitCount(),
				new AdminsDto(item.getAdmins().getEmail(), item.getAdmins().getFirstName(),
						item.getAdmins().getLastName(), null, false, null),
				this.getImageDtoList(item.getImages()), this.getCommentDtoList(item.getComments()),
				new CategoryDto(item.getCategory().getCategoryId(), item.getCategory().getCategoryName(), null))));
		return newsList;
	}
...
//	}
	...
	public List<ImageDto> getImageDtoList(List<Image> images) {
		...
		images.forEach(img -> imagesDto.add(new ImageDto(img.getImageId(), img.getImageUrl(), null)));
		return imagesDto;
	}
	...
	public List<CommentDto> getCommentDtoList(List<Comment> comments) {
		...
		comments.forEach(com -> commentsDto.add(
				new CommentDto(com.getCommentId(), com.getPostedBy(), com.getPostedDate(), com.getComment(), null)));
		return commentsDto;
	}
